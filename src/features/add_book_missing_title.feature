...
# - Knappen "Lägg till ny bok" ska vara inaktiverad tills både titel och författare är ifyllda.
  ...
  Feature: Lägga till ny bok med endast författare ifylld
  Som en användare
  vill jag inte kunna lägga till en ny bok i katalogen om jag endast anger författare
  så att jag inte får ofullständiga böcker i katalogen

    Scenario: Försöka lägga till en ny bok med endast författare ifylld
      Given användaren navigerar till "Lägg till bok"
      When ett formulär visas
      And användaren fyller i "Författare" med "test"
